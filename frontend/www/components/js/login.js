    ...
    return new Promise(function(resolve, reject) {
        ...
        ConnectyCube.createSession(function(csErr, csRes) {
            var userRequiredParams = {
                'login':localStorage.c_name,
                'password': localStorage.c_pass
            };
            if (csErr) {
                loginError(csErr);
            } else {
...
ConnectyCube.users.signup(userProfile, function(error, user){
        ...
        function loginSuccess(userData){
            ...
            ConnectyCube.chat.connect({userId:  localStorage.chatid, password: localStorage.password}, function(err, roster){
                if (err) {
