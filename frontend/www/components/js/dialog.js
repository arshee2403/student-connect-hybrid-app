...
Dialog.prototype.loadDialogs = function (type) {
    ...
    return new Promise(function(resolve, reject){
        ...
        if (type === 'chat') {
            filter['type[in]'] = [CONSTANTS.DIALOG_TYPES.PRIVATE, CONSTANTS.DIALOG_TYPES.GROUPCHAT].join(',');
        } else {
            filter.type = CONSTANTS.DIALOG_TYPES.PUBLICCHAT;
        }

        ConnectyCube.chat.dialog.list(filter, function (err, resDialogs) {
            if (err) {
                reject(err);
            }
...
Dialog.prototype.renderDialog = function (dialog, setAsFirst) {
    ...
    if (dialog.type !== CONSTANTS.DIALOG_TYPES.PRIVATE && !dialog.joined) {
...
Dialog.prototype.joinToDialog = function (id) {
    ...
    return new Promise(function (resolve, reject){
        ConnectyCube.chat.muc.join(jidOrUserId, function (resultStanza) {
...
Dialog.prototype.renderMessages = function (dialogId) {
    ...
    if (!document.forms.send_message) {
        ...
        if(dialog.type === CONSTANTS.DIALOG_TYPES.PRIVATE || dialog.type === CONSTANTS.DIALOG_TYPES.GROUPCHAT) {
...
Dialog.prototype.createDialog = function (params) {
    ...
    ConnectyCube.chat.dialog.create(params, function (err, createdDialog) {
        if (err) {
          alert(JSON.stringify(err));
            ...
            for (var i = 0; i < newOccupantsIds.length; i++) {
                ConnectyCube.chat.sendSystemMessage(newOccupantsIds[i], systemMessage);
            }
            ...
            if(createdDialog.type !== CONSTANTS.DIALOG_TYPES.PRIVATE){
...
Dialog.prototype.getDialogById = function (id) {
    return new Promise(function(resolve, reject){
        ...
        ConnectyCube.chat.dialog.list({"_id": id}, function (err, res) {
            if (err) {
                console.error(err);
                reject(err);
            }
...
Dialog.prototype.updateDialog = function (updates) {
    ...
    function _sendUpdateStanza (){
        return new Promise (function(resolve, reject){
            ConnectyCube.chat.dialog.update(dialogId, toUpdateParams, function(err, dialog) {
                if (err) {
                    reject(err);
                } else {
                    resolve(dialog);
    ...
    function _notifyNewUsers (users) {
        ...
        _.each(users, function(user){
            ConnectyCube.chat.sendSystemMessage(+user, msg);
        });
    }
};
...
Dialog.prototype.quitFromTheDialog = function(dialogId){
    ...
    switch (dialog.type){
        case CONSTANTS.DIALOG_TYPES.PUBLICCHAT:
            ...
            break;
        case CONSTANTS.DIALOG_TYPES.PRIVATE:
            ConnectyCube.chat.dialog.delete([dialogId], function(err) {
                if (err) {
                    console.error(err);
                } else {
        ...
        case CONSTANTS.DIALOG_TYPES.GROUPCHAT:
            ...
            ConnectyCube.chat.dialog.update(dialogId, {
                pull_all: {
